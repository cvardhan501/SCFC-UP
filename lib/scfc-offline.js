...
(function (window) {
  ...
  class SCFCDB {
    ...
    async saveUserData(usn, data) {
      ...
      const cleanUsn = String(usn).toUpperCase();
      try {
        const existing = await this.getUserData(cleanUsn);
  ...
  class SCFCSyncEngine {
    ...
    getActiveUSN() {
      if (typeof state !== 'undefined' && state && state.usn) {
        return String(state.usn).trim().toUpperCase();
      }
      return '';
    }
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (!contentType.includes('application/json')) {
            console.warn('[SyncEngine] Non-JSON response received during sync');
          ...
          // Update local data with confirmed server response if matching USN
          if (json.student && json.student.usn && json.student.usn.toUpperCase() === usn) {
    ...
    async handleSyncFailure(pendingItems) {
      ...
      const activePending = pendingItems.filter(i => i.status === 'pending');
      this.notifyStatus('pending', activePending.length ? `${activePending.length} change${activePending.length > 1 ? 's' : ''} pending sync (will retry)` : 'Offline Mode', activePending.length);
    }
    ...
    async pullServerState(usn) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const json = await res.json();
          if (json.student && json.student.usn && json.student.usn.toUpperCase() === usn) {
            const existingLocal = await this.db.getUserData(usn);
            const serverStr = JSON.stringify(json.student);
