...
const getGmailSenderEmail = () => {
  return process.env.GMAIL_SENDER_EMAIL || 'scfc.studentos@gmail.com';
};

/**
 * Constructs RFC 2822 Base64URL-encoded MIME email for Gmail API sending
 */
function createBase64UrlMimeMessage({ to, from, subject, htmlContent, textContent }) {
  const utf8Subject = `=?utf-8?B?${Buffer.from(subject).toString('base64')}?=`;
  const messageParts = [
    `From: SCFC StudentOS <${from}>`,
    `To: ${to}`,
    `Subject: ${utf8Subject}`,
    'MIME-Version: 1.0',
    'Content-Type: multipart/alternative; boundary="===SCFC_BOUNDARY_==="',
    '',
    '--===SCFC_BOUNDARY_===',
 ...
 */
async function sendViaGmailApi({ toEmail, subject, htmlContent, textContent }) {
  ...
  try {
    ...
    const fromEmail = getGmailSenderEmail();
    const raw = createBase64UrlMimeMessage({
      to: toEmail,
      from: fromEmail,
      subject,
      htmlContent,
    ...
    if (res && res.data && res.data.id) {
      console.log(`✅ Email sent via Gmail API to ${toEmail} (ID: ${res.data.id})`);
      return { success: true, id: res.data.id, provider: 'gmail' };
    } else {
      return { success: false, error: 'Gmail API response missing message ID' };
    }
  } catch (error) {
    console.error('❌ Gmail API delivery error:', error.message || error);
    return { success: false, error: error.message || 'Gmail API delivery failed' };
  }
}
...
async function sendViaResend({ toEmail, subject, htmlContent, textContent }) {
  ...
  if (!resend) {
    ...
    return { success: false, error: 'Email service is not configured. Please set RESEND_API_KEY or Gmail OAuth credentials.' };
  }
 ...
 * Attempts primary delivery via Gmail API. If Gmail API is unconfigured or fails, falls back to Resend.
 */
async function dispatchEmail({ toEmail, subject, htmlContent, textContent }) {
  if (toEmail.endsWith('@example.com') || toEmail.endsWith('@test.com') || toEmail.endsWith('@test.local')) {
    console.log(`[Email Service] Mock delivery to test domain: ${toEmail}`);
    return { success: true, messageId: 'mock-test-id', provider: 'mock' };
  }

  // Primary: Gmail API
  const gmailResult = await sendViaGmailApi({ toEmail, subject, htmlContent, textContent });
  if (gmailResult.success) {
    return gmailResult;
  }

  if (gmailResult.reason !== 'GMAIL_NOT_CONFIGURED') {
    console.warn(`⚠️ Gmail API dispatch failed (${gmailResult.error}). Falling back to Resend...`);
  }
...
SCFC Team
  ...
  return await dispatchEmail({ toEmail, subject, htmlContent, textContent });
}
...
/**
 * Send Change Email Address Verification
 */
